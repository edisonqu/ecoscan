...
router.get('/:number', async function (req, res, next) {
        try {
            ...
            const index = unknown_data.findIndex(item => item.code === data.code);
            if(index !== -1){
                const datas = {
                    code: unknown_data[index].code,
                    status_verbose: data.status_verbose,
                    image: data.product.image_url,
                    name: unknown_data[index].name,
                    ecoscore: unknown_data[index].ecoscore,
                    nutrition: data.product.nutrition_grades,
                    keywords: data.product._keywords,
                    packaging: unknown_data[index].packaging
                }
                res.send(datas)
            }
            else {
